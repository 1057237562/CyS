...
import numpy as np
import lora.utils as lora_utils
from mlx.utils import tree_flatten
import mlx_lm.utils as utils
from lora.models import LoRALinear
...
def train(model, train_set, val_set, optimizer, loss, tokenizer, iters=1000, batch_size=4, steps_per_report=10, steps_per_eval=200, adapter_file="adapter.npz", save_every=10, val_batches=1):
...
def generate(model, prompt, tokenizer, sampler, max_tokens):
    ...
    skip = 0
    for token, n in zip(
        lora_utils.generate(prompt, model, sampler),
        range(max_tokens),
    ):
...
def run_lora(model_ref, data_path, adapter_file, learning_rate=1e-5, resume_adapter_file=None, tokenizer_config={}, lora_layers=16, seed = 0):
    np.random.seed(seed)
    # Building tokenizer_config

    print("Loading pretrained model")
    model, tokenizer = utils.load(model_ref, tokenizer_config)
    ...
    train_set, valid_set, test_set = load(data_path)
...
#     print("Loading datasets")
#     train_set, valid_set, test_set = load(args.data)
